...
Profile.pull = function(user, callback) {
  ...
  async.forEachOfSeries(profiles, function(profileID, profileName, nextProfile) {
    Profile.findOne(profileID, function(err, profile) {
      ...
      // var requestOpts = google.requestOpts(user, 'data', 'ga', {
      //       ids: 'ga:'+profile.id,
      ...
      // var requestOpts = fb.requestOpts(user, profileID, { fields: 'likes' });
      //
      // profile.modifyByDelta(function(err, profile) {
      //   if (err) return next(err);
      //
      //   fb.request(requestOpts, function (err, body) {
      //     if (err) return next(err);
      //
      //     if (profile.likes == body.likes) {
      //       return next();
      //     }
      //
      //     profile.createDelta('likes', body.likes, next);
      //   });
      // });
      nextProfile();
    });
      ...
      async.whilst(function() { return midMonthDate !== null; },
        function(done) {
          ...
          google.pager('nextLink', requestOpts, function(row, next) {
            ...
            bulkUpdates.push(DELTA_BULK_CREATE_ACTION);
            bulkUpdates.push(buildBulkCreateDelta(profile, row, data, 'sessions', 4));
            bulkUpdates.push(DELTA_BULK_CREATE_ACTION);
            bulkUpdates.push(buildBulkCreateDelta(profile, row, data, 'bounces', 5));
            bulkUpdates.push(DELTA_BULK_CREATE_ACTION);
            bulkUpdates.push(buildBulkCreateDelta(profile, row, data, 'pageViews', 6));
            bulkUpdates.push(DELTA_BULK_CREATE_ACTION);
            bulkUpdates.push(buildBulkCreateDelta(profile, row, data, 'users', 7));
            bulkUpdates.push(DELTA_BULK_CREATE_ACTION);
            bulkUpdates.push(buildBulkCreateDelta(profile, row, data, 'sessionDuration', 8));
