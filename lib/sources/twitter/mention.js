...
function processReply(user, rawReply, callback) {
  Tweet.findOne(rawReply.in_reply_to_status_id_str, function(err, tweet) {
    ...
    if (tweet) {
      ...
      tweet.createDelta('reply_count', reply_count, function() {
        callback();
      });
    } else {
      Tweet.pullOne(user, rawReply.in_reply_to_status_id_str, function(err) {
        ...
        processReply(user, rawReply.in_reply_to_status_id_str, function() {
          return callback();
        });
...
function actuallyPopulateFields(user, mention, rawMention) {
  ...
  if(rawMention.coordinates) {
    location.lat = rawMention.coordinates.coordinates[0];
    location.lon = rawMention.coordinates.coordinates[1];
  } else {
    location = null;
  }
  _.extend(mention, {
