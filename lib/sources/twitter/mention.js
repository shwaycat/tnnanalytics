  ...
  async.doWhilst(
    function(cb) {
      Mention.pull(user, maxID, function(err, tweets) {
        console.log(err);
        // if(err == RATE LIMIT) return setTimeout(20min cb);
...
function processReply(user, rawReply, callback) {
  Tweet.findOne(rawReply.in_reply_to_status_id_str, function(err, tweet) {
    ...
    if (tweet) {
      ...
      Tweet.pull(user, rawReply.in_reply_to_status_id_str, function(err) {
...
Mention.prototype.populateFields = function(user, rawMention, callback) {
  ...
  if(rawMention.in_reply_to_user_id_str == user.services.twitter.profileId) {
