...
function DirectMessage(id, obj) {
  AbstractType.Base.call(this, "Twitter", id, obj)
...
DirectMessage.pull = function(user, callback) {
  ...
  var client = new twitter({
    ...
    , params = {count: 200, include_entities: 0, since_id: null};
  ...
  // client.get('statuses/mentions_timeline', params, function(err, tweets, response) {
    ...
    debug('%s direct-messages for user %s', tweets.length, user.id)
    ...
    async.eachLimit(messages, 5, function(message, next) {
      ...
      if (err) {
        ...
        debug("updating dmSinceId for %s to %s", user.id, tweets[0].id_str)
