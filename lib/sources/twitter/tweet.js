...
Tweet.pull = function(user, maxID, callback) {
  ...
  if (user.services.twitter.tweetSinceId && user.services.twitter.tweetSinceId != '' && !argv.all) {
          ...
          if(res.status == 409) {
            ...
            Tweet.findOneWithDelta(tweet.id_str, function(err, foundTweet) {
              ...
              if(foundTweet) {
                ...
                debug("Old Tweet Favorites: %s", foundTweet.retweet_count);
                debug("Found Tweet Favorites: %s", tweet.retweet_count);
