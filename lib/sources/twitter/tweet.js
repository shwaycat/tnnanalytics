...
/**
 * Pull One Twitter Tweet By ID -- NO LONGER USED
    ...
    function(cb) {
      Tweet.pull(user, maxID, function(err, tweets) {
        if(err && err[0]['code'] == 88) {
          debug('Rate limit exceeded for Tweets');
        ...
        DELTA_FIELDS.forEach(function(fieldName) {
          ...
          if(foundTweet[fieldName] != tweet[fieldName]) {
            series.push(function(callback) {
              foundTweet.createDelta(user, fieldName, tweet[fieldName], function(err, res) {
                callback(err, res);
              });
            });
          }
 ...
 */
Tweet.prototype.createDelta = function(user, key, value, timestamp, callback) {
