...
function createPost(user, data, callback) {
  var post = new Post(data.id, {
        timestamp: new Date(data.created_time),
        cadence_user_id: user.id,
        doc_text: data.message,
        doc_source: DOC_SOURCE,
        likes: 0,
        shares: 0
      });

  if (data.likes && data.likes.summary) {
    post.likes = data.likes.summary.total_count;
  }

  if (data.shares) {
    post.shares = data.shares.count;
  }
...
function bulkUpdatePost(post, data, bulkUpdater, callback) {
  post.modifyByDelta(function(err, post) {
    ...
    bulkUpdater.flushIfFull(callback);
  });
}
    ...
    function(done) {
      Post.findLatest(user, function(err, latestPost) {
        ...
        fb.pager('next', requestOpts, function(obj, next) {
          ...
          createPost(user, obj, function(err) {
            if (err && err.status == 409) return next(null, null);
            next(err);
          });
        }, done);
      });
      ...
      fb.request(requestOpts, function(err, body) {
        ...
        async.eachSeries(body.data, function(data, next) {
          ...
          Post.findOne(data.id, function(err, post) {
            ...
            if (!post) {
              var getPostRequestOpts = fb.requestOpts(user, data.id, {
                    fields: 'message,created_time,shares,likes.summary(true).limit(0)'
                  });

              fb.request(getPostRequestOpts, function(err, body) {
                if(err) return next(err);

                createPost(user, body, function(err, post) {
                  if(err) return next(err);
                  bulkUpdatePost(post, data, bulkUpdater, next);
                });
              });
            } else {
              bulkUpdatePost(post, data, bulkUpdater, next);
