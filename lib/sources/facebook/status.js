...
function makeStatusFromHit(hit) {
  return new Status(hit._id, hit._source);
}
...
Status.findLatest = function(user, callback) {
  keystone.elasticsearch.search({
    ...
    if (err) return callback(err);
    if (res.hits != 1) return callback(null, null);
...
Status.pull = function(user, callback) {
  ...
  Status.findLatest(user, function(err, latestStatus) {
    requestOpts = fb.requestOpts(user, pageID, 'statuses', {
      fields: 'message,created_time,likes.summary(true).limit(0)',
      since: Math.floor(latestStatus.timestamp.getTime()/1000)
    });
  });

  debug("pulling new statuses for user id %s, page %s", user.id, user.services.facebook.pageID);

  fb.pager('next', requestOpts, function(obj, next) {
    if (!obj) return next(false);
