        ...
        async.eachSeries(body.data, function(data, next) {
          ...
          Status.findOne(data.id, function(err, status) {
            ...
            status.modifyByDelta(function(err, status) {
              ...
              if (data.likes.summary.total_count && data.likes.summary.total_count != status.likes) {
                bulkUpdater.addSource(status.deltaBody('likes', data.likes.summary.total_count));
              }
