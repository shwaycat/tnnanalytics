var _ = require('underscore'),
    ...
    async = require('async'),
    FacebookAPIError = require('../../facebook-api-error'),
...
exports.request = function(requestOpts, callback) {
  ...
  request(requestOpts, function (err, res, body) {
    if (!err && 'object' === typeof body.error) {
      err = new FacebookAPIError(body);
    }
    callback(err, body);
  });
};
  ...
  async.whilst(
    ...
    function(cb) { // iterator
      ...
      request(requestOpts, function (err, res, body) {
        ...
        if (!body.data) {
          console.warn("Missing body.data in facebook response: %j (Request opts: %j)",
            body, requestOpts);
          requestOpts = null;
          return cb();
        }
