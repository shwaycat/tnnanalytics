...
/**
 * Geta the lastest delta object for the given key
 * @param {string} key - Field name
 * @param {esCreateCallback} callback
 */
Page.prototype.getDelta = function(key, callback) {
  keystone.elasticsearch.search({
    ...
    if (!res.hits || !res.hits.total) {
      ...
      callback(null, res.hits.hits[0]._source);
    }
  });
};
...
Page.pull = function(user, callback) {
  ...
  Page.findOne(pageID, function(err, page) {
    ...
    async.series([
      function(done) {
        page.getDelta('likes', function(err, delta) {
          if (err) return done(err);
          if (delta) {
            page.likesLastTimestamp = new Date(delta.timestamp);
          } else {
            page.likesLastTimestamp = page.timestamp;
          }
          done();
        });
      },
      function(done) {
        var requestOpts = fb.requestOpts(user, page.id, 'insights', 'page_fans', 'lifetime');

        fb.pager('previous', requestOpts, function(obj, next) {
          if (!obj.value) return next('stop');

          obj.endTime = new Date(obj.end_time);

          if (page.likesLastTimestamp >= obj.endTime) return next('stop');

          page.createDelta('likes', obj.value, obj.endTime, next);
        }, done);
      },
      function(done) {
        page.getDelta('likesByCountry', function(err, delta) {
          if (err) return done(err);
          if (delta) {
            page.likesByCountryLastTimestamp = new Date(delta.timestamp);
          } else {
            page.likesByCountryLastTimestamp = page.timestamp;
          }
          done();
        });
      },
      function(done) {
        ...
        fb.pager('previous', requestOpts, function(obj, next) {
          ...
          var objKeys = _.keys(obj.value);

          if (!objKeys.length) return next('stop');

          obj.endTime = new Date(obj.end_time);

          if (page.likesByCountryLastTimestamp >= obj.endTime) return next('stop');

          async.eachSeries(objKeys, function(key, nextKey) {
            var data = {
              country: key,
              likesByCountry: obj.value[key]
            };

            page.createDelta('likesByCountry', data, obj.endTime, nextKey);
          }, next);
        }, done);
      },
      function(done) {
        page.getDelta('impressions', function(err, delta) {
          if (err) return done(err);
          if (delta) {
            page.impressionsLastTimestamp = new Date(delta.timestamp);
          } else {
            page.impressionsLastTimestamp = page.timestamp;
          }
          done();
        });
      },
      function(done) {
        ...
        fb.pager('previous', requestOpts, function(obj, next) {
          ...
          if (page.impressionsLastTimestamp >= obj.endTime) return next('stop');

          page.createDelta('impressions', obj.value, obj.endTime, next);
        }, done);
      }
    ], callback);
  });
