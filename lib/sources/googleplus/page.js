...
Page.pull = function(user, callback) {
  ...
  gplus.request(requestOpts, function(err, body) {
    ...
    if(body.error && body.error.code == 401) {
      ...
      getAccessToken(user.services.googleplus.refreshToken, function(err, access_token) {
        if(err) return callback(err);

        user.services.googleplus.accessToken = access_token;
...
Page.prototype.emailLinkObject = function(opts) {
  return {
    ...
    href: 'http://plus.google.com/' + opts.user.services.googleplus.profileId
  };
};
