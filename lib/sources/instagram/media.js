...
function Media(id, obj) {
  AbstractType.call(this, "Instagram", id, obj);
  ...
  Media.findOne(media.id, function(err, esMedia) {
    ...
    if(esMedia) {
      ...
      esMedia.modifyByDelta(function(err){
        ...
        DELTA_FIELDS.forEach(function(fieldMap) {
          ...
          if(esMedia[esKey] != instaValue) {
            series.push(function(callback) {
              esMedia.createDelta(user, esKey, instaValue, function(err, res) {
                callback(err, res);
              });
            });
          }
        ...
        if(series.length) {
          async.series(series, function(err, results) {
            debug('%s Deltas created for Media %s', series.length, media.id);
            callback(err, results);
          });
        } else {
          debug('No deltas created for Media %s', media.id);
          callback();
        }
...
Media.prototype.populateFields = function(user, rawMedia) {
  ...
  if(rawMedia.retweeted_status) {
    this.isRetweet = true;
  }

  _.extend(this, {
    doc_text: rawMedia.text,
    ...
    timestamp: new Date(rawMedia.created_at)
  });
}
