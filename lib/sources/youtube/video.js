var util = require("util"),
    ...
    DELTA_FIELDS = [ 'viewCount', 'likeCount', 'dislikeCount', 'commentCount' ];
...
Video.pull = function(user, callback) {
  ...
  if(user.services.googleplus.youtubeChannelUploadPlaylistID) {

    var requestOpts = yt.requestOpts(user, 'playlistItems', {
      part: 'contentDetails',
      playlistId: user.services.googleplus.youtubeChannelUploadPlaylistID,
      maxResults: 50
    });
...
Video.prototype.populateFields = function(user, rawVideo) {
  _.extend(this, {
    ...
    cadence_user_id: user.id,
    user_id: rawVideo.snippet.channelId,
    user_name: rawVideo.snippet.channelTitle,
