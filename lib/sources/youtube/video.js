  ...
  pullVideoIDsInGroups(user, 50, function(videoIDs, nextGroup) {
    ...
    yt.pager('next', requestOpts, function(data, next) {
      ...
      async.series([
        ...
        function(nextTask) {
          async.forEachOfSeries(videos, function(video, videoID, nextVideo) {
            var video0 = _.clone(video);

            video.modifyByDelta(function(err, video) {
              if (err) return nextVideo(err);

              video.viewCount -= video0.viewCount;
              if (video.viewCount) {
                bulkUpdater.addSource(video.deltaBody('viewCount', video.viewCount));
              }

              video.likeCount -= video0.likeCount;
              if (video.likeCount) {
                bulkUpdater.addSource(video.deltaBody('likeCount', video.likeCount));
              }

              video.dislikeCount -= video0.dislikeCount;
              if (video.dislikeCount) {
                bulkUpdater.addSource(video.deltaBody('dislikeCount', video.dislikeCount));
              }

              video.commentCount -= video0.commentCount;
              if (video.commentCount) {
                bulkUpdater.addSource(video.deltaBody('commentCount', video.commentCount));
              }

              bulkUpdater.flushIfFull(nextVideo);
            });
          }, function(err) {
            if (err) return nextTask(err);
            bulkUpdater.flush(nextTask);
