...
exports.authenticateUser = function(req, res, next) {
  ...
  if (_.has(req.query, 'cb')) {
    ...
    passport.authenticate('google', { session: false }, function(err, data) {
      ...
      getYoutubeChannelID(data.accessToken, function(err, channelID) {
        if (err) return next(err);

        if (req.user) {
          serviceData.youtubeChannelID = channelID;
...
function getYoutubeChannelID(accessToken, callback) {
  request({
    ...
    callback(null, _.first(body.items).id);
  });
}
...
//       fields: 'items(id,name),nextLink,totalResults'
//     },
//     json: true
//   }, function (err, res, body) {
//     if (err) return callback(err);
//
//     callback(null, { analyticsAccounts: body.items });
//   });
// };
