...
function extractDataPoint(bucket) {
  var facebook = (bucket.facebook.engagement.value || 0),
  ...
  var facebookBuckets = mxm.objTry(bucket, 'facebook', 'docs', 'buckets'),
      twitterBuckets = mxm.objTry(bucket, 'twitter', 'docs', 'buckets');

  if (facebookBuckets && facebookBuckets.length) {
    _.each(facebookBuckets, function(obj) {
      ...
      if(buckets && buckets.length) {
        ...
        return callback(null, {
          ...
          summary: {
            ...
            "changeInEngagement" : _.last(dataReturn).value - _.first(dataReturn).value
          }
        });
      } else {
        return callback(null, {
