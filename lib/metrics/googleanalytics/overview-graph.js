...
module.exports = function(user, profileName, startTime, endTime, callback) {
  ...
  User.model.getAccountRootInfo(user.accountName, function(err, accountRoot) {
    ...
    keystone.elasticsearch.search({
      ...
      if(aggs) {
        _.extend(summary, {
          totalPageViews: mxm.objTry(aggs, 'pageViews', 'value') || 0,
          totalUsers: mxm.objTry(aggs, 'users', 'value') || 0,
          totalBounces: mxm.objTry(aggs, 'bounces', 'value') || 0,
          totalSessionDuration: mxm.objTry(aggs, 'sessionDuration', 'value') || 0
        });
